...
import com.gumio_inf.aacsampleapp.ui.viewmodel.ClockViewModel
...
class LiveDataActivity : AppCompatActivity() {
    ...
    inner class SensorHandler(private val mSensorManager: SensorManager): SensorEventListener, LifecycleObserver {
        ...
        fun stop(source: LifecycleOwner) {
            lifecycle.removeObserver(this)
            Log.d("ObserverStatus", "${this}の購読を切りました")
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val dateObserver = Observer<String> {
            mBinding.clockText.text = mBinding.clockModel?.strTime?.value
        }
        ...
        val observer = object : LifecycleObserver {
            ...
            fun calledWhenOnStart(source: LifecycleOwner) {
                ...
                mBinding.clockModel?.strTime?.observeForever(dateObserver)
            }
            @OnLifecycleEvent(Lifecycle.Event.ON_STOP)
            fun calledWhenOnStop(source: LifecycleOwner) {
                mBinding.clockModel?.strTime?.removeObservers(source)
            }
            @OnLifecycleEvent(Lifecycle.Event.ON_DESTROY)
